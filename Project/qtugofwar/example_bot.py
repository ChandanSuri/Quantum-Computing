...
'''
    ...
    This approach would work well when our team gets at least the same number of X gates as the opponent team. Also, it would work well when the probability distribution is very close to half and this approach using H gates can benefit our team efficiently. In some instances, first Z gate would be applied before applying H gate when near the end of the game, this would also benefit us in the case when the state have coefficients that are opposite in sign.
    
    This approach would fail for the cases when the opponent team has more X gates or is the last one to end the game. In those cases, it can swap the probabilities making us lose. Also, if some opponent always applies the same gates as us in the final rounds when we are trying to swap the coefficients, then this approach could also fail depending on the hand the opponent has.
'''
class MyStrategy(GameBot):
    ...
    def play_action(self,
        ...
        if team == 0:
            usable_cards = [GameAction.PAULIZ, GameAction.REVERSE, GameAction.MEASURE, GameAction.HADAMARD]
