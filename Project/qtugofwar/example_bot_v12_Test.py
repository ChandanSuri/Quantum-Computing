...
class MyStrategy(GameBot):
    ...
    def play_action(self,
        ...
        usable_cards_0 = [GameAction.REVERSE, GameAction.MEASURE, GameAction.PAULIZ, GameAction.HADAMARD]
        sig_cards_0 = [GameAction.PAULIX, GameAction.HADAMARD, GameAction.PAULIZ]
        
        usable_cards_1 = [GameAction.PAULIZ, GameAction.REVERSE, GameAction.MEASURE, GameAction.HADAMARD]
        sig_cards_1 = [GameAction.PAULIX, GameAction.HADAMARD]
        
        if team == 0:
            usable_cards = usable_cards_0
            sig_cards = sig_cards_0
        else:
            usable_cards = usable_cards_1
            sig_cards = sig_cards_1
        ...
        if team == 0:
            ...
            if round_number >= rounds_threshold:
                self.last_played_action = prev_turn['team0_action'] if prev_turn['team1_action'] is None else None
        else:
            self.is_direction_positive = not(self.is_direction_positive) if prev_turn['team1_action'] == GameAction.REVERSE else self.is_direction_positive
            direction = 1 if self.is_direction_positive else -1
            self.current_state = self.perform_action(self.current_state, direction, prev_turn['team1_action'], prev_turn['team1_measurement'], True)
            ...
            self.current_state = self.perform_action(self.current_state, direction, prev_turn['team0_action'], prev_turn['team0_measurement'], False)
            if round_number >= rounds_threshold:
                self.last_played_action = prev_turn['team1_action'] if prev_turn['team0_action'] is None else None
            ...
            if round_number >= rounds_threshold:
                if self.current_state[0]**2 > self.current_state[1]**2:
                    ...
                    for curr_hand in hand:
                        if curr_hand in sig_cards and curr_hand not in state_changes_map and curr_hand != self.last_played_action:
